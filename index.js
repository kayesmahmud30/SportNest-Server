...
// 🚫 app.listen() BAAD DEWA HOYCHE (Vercel automatic port handle korbe)
...
// Database connection & collection setup shortcut (Serverless friendly)
const db = client.db("sportnest");
const db_col = db.collection("facilities_data");
const db_col_2 = db.collection("bookings");
...
app.get("/facility", async (req, res) => {
  try {
    ...
    if (sport && sport !== "All Facilities") {
      const sportFilter = Array.isArray(sport) ? sport : [sport];
      query.facilityType = { $in: sportFilter };
    }
...
app.delete("/facility/:id", verify, async (req, res) => {
  const id = req.params.id;
  const result = await db_col.deleteOne({ _id: new ObjectId(id) });
  res.send(result);
});
...
// 🚀 Vercel er jonno app export kora holo
module.exports = app;
